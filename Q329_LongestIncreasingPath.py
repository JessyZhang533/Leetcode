def longestIncreasingPath(self, matrix: List[List[int]]) -> int:
    ...
    visited = [[0]*N for _ in range(M)]  # 2-D matrix for storing the number of steps

    # dfs function
    def dfs(i, j):
        if not visited[i][j]:  # if this position is not visited
            val = matrix[i][j]
            ...
            visited[i][j] = 1 + max(up, down, left, right)
        return visited[i][j]  # !!!: so the cells visited in 'visited' will be numbered as '1 2 3 ...', which shows the path 1-->2-->3-->...
